const panelData = {
    vault: { 
        title: '정보금고 기반 위험 평가', 
        content: '<p>개인 및 단체의 Data Vault(정보금고)에서 익명화된 활동 기록을 토대로 위험도를 산정합니다.</p><p><strong>오픈해시 기술이 모든 데이터의 진실성을 보장합니다.</strong> SHA-256 이중 재해싱과 BLS 서명 검증을 통해 건강검진 기록, 운동량, 운전습관 등 모든 데이터가 위변조 없이 원본 그대로임을 증명합니다.</p><p>오염된 노드는 5ms 이내 격리되고, Merkle Tree 구조로 누구나 데이터를 검증할 수 있습니다. 오픈해시는 "디지털 공증인"으로서 기록 시점을 증명하고 이후 변경을 원천 차단합니다.</p>' 
    },
    unified: { 
        title: '통합 보험 시스템', 
